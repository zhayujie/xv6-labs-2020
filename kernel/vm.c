...
void
kvmmap(pagetable_t process_kernel_pagetable, uint64 va, uint64 pa, uint64 sz, int perm)
{
...
void
free_kernel_pagetable(pagetable_t pagetable, int level) {
  for (int i = 0; i < 512; i++) {
    pte_t pte = pagetable[i];
    if (pte & PTE_V && level < 2) {
      uint64 child = PTE2PA(pte);
      free_kernel_pagetable((pagetable_t) child, level+1);
    }
  }
  kfree((void*) pagetable);
}
